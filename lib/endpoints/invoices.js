...
class Invoices {
	...
	/**
	 * @param {string} id Mark a pending invoice completed.
	 */
    async markInvoiceCompleted(id) {
        try {
            const params = `invoices/${id}/mark-completed`;
            return await makeRequest(this.url, params, null, "PATCH", this.api_key, this.store);
        } catch (error) {
            throw error;
        }
    }
	...
	/**
	 * @param {string} id Mark a pending invoice void.
	 */
    async markInvoiceVoid(id) {
        try {
            const params = `invoices/${id}/mark-void`;
            return await makeRequest(this.url, params, null, "PATCH", this.api_key, this.store);
        } catch (error) {
            throw error;
        }
